...
export const getStory = (userId) => {
    const selectStory = 
    `SELECT idStory, title, synopsis, date_format(creationDate, '%d/%m/%Y'), exposition, risingAction, climax, fallingAction, resolution, genreId, genreName FROM story INNER JOIN genre on genre.idGenre = story.genreId WHERE userId = ?;`;
    return db.query(selectStory, [userId]);
}

export const getStoryById = (idStory, userId) => {
    const selectStoryById = 
    `SELECT idStory, title, synopsis, date_format(creationDate, '%d/%m/%Y'), exposition, risingAction, climax, fallingAction, resolution, genreId, genreName FROM story INNER JOIN genre on genre.idGenre = story.genreId WHERE idStory = ? AND userId = ?;`;
...
export const updateStory = (title, synopsis, creationDate, genreId, exposition, risingAction, climax, fallingAction, resolution, idStory, userId) => {
    ...
    'UPDATE story SET title=? , synopsis=?, creationDate=?, genreId =?, exposition=?, risingAction=?, climax=?, fallingAction=?, resolution=? WHERE userId = ? AND idStory = ?;';
